...
@section('content')
    <div class="container">
        <div class="justify-items-center mb-10">
            <h2 class="text-4xl font-extrabold dark:text-black">Edit Task</h2>
        </div>
        <div class="">
        ...
        <form class="max-w-md mx-auto" action="{{ route('task.update', [$task->id]) }}" method="post">
            @csrf
            {{ method_field('PUT') }}
            <div class="relative z-0 w-full mb-5 group">
                <input type="text" value="{{ $task->task }}" name="task" id="floating_task" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
